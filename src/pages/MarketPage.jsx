...
export default function MarketPage() {
  ...
  const filteredRaises = useMemo(() => raises.filter(r => {
  ...
  }), [raises, activeRounds, activeSectors])
  ...
  return (
    <div className="space-y-8">
      <div>
        <h1 className="text-2xl font-bold tracking-tight text-[#fafafa]">Venture Capital Funding</h1>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        <StatCard label="Total Raised"   value={formatAmount(stats.total)}            sub={`${filteredRaises.length} rounds`} accent="#6366f1" />
        <StatCard label="Avg Deal Size"  value={formatAmount(Math.round(stats.avgDeal))} />
        <StatCard label="Largest Round"  value={formatAmount(stats.largest)} />
        <StatCard label="Companies"      value={stats.companies} sub="unique" />
      </div>
      ...
      <div className="rounded-2xl border border-[#27272a] bg-[#18181b] p-5 space-y-5">
        ...
        <FilterBar
          ...
          activeRounds={activeRounds} setActiveRounds={setActiveRounds}
          activeSectors={activeSectors} setActiveSectors={setActiveSectors}
          chartType={chartType} setChartType={setChartType}
        />
        <FundingChart
          period={period} activeRounds={activeRounds}
          raises={filteredRaises} chartType={chartType}
        />
      </div>
