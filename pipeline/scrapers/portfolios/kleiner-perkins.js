...
const FIRM_ID = 'kleiner'
const URL = 'https://www.kleinerperkins.com/companies/'

export async function scrapeKleinerPerkins() {
  const db = getDb()
  ...
  const companies = await scrapeWithPlaywright(URL, async (page) => {
    ...
    return page.evaluate(() => {
      const items = []
      document.querySelectorAll('[class*="company"], [class*="portfolio"], article, [class*="card"]').forEach(el => {
        ...
        const sector = el.querySelector('[class*="sector"], [class*="tag"], [class*="category"]')?.textContent?.trim()
