...
const FIRM_ID = 'gv'
const URL = 'https://www.gv.com/portfolio/'

export async function scrapeGV() {
  const db = getDb()
  ...
  const companies = await scrapeWithPlaywright(URL, async (page) => {
    ...
    return page.evaluate(() => {
      const items = []
      document.querySelectorAll('[class*="company"], [class*="portfolio"], [class*="card"], a[href]').forEach(el => {
        const name = el.querySelector('h2, h3, h4, [class*="name"]')?.textContent?.trim()
        const desc = el.querySelector('p')?.textContent?.trim()
        if (name && name.length > 1 && name.length < 80) items.push({ name, desc })
      })
